...
} from '@/features/events/state/events-api-slice';
...
export default function SingleEventDetailsPage() {
  ...
  const { data: userEventRole, isLoading: isLoadingUER } = useCheckUserEventRoleQuery({
    eventId: eventId ?? '',
  });
  const { data: authData } = authClient.useSession();
  const navigate = useNavigate();

  // Prevent users who are not part of an event from viewing
  useEffect(() => {
    if (isLoading || isLoadingUER) return;
  ...
  }, [userEventRole, authData, navigate, isLoading, isLoadingUER]);
